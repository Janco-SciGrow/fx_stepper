...
class FxStep {
  ...
  const FxStep({
    ...
    this.state = FxStepState.indexed,
    this.isActive = false,
    this.label,
  });
  ...
  final FxStepState state;

  /// Whether or not the step is active. The flag only influences styling.
  final bool isActive;

  /// Only [FxStepperType.horizontal], Optional widget that appears under the [title].
...
class FxStepper extends StatefulWidget {
  ...
  const FxStepper({
    ...
    required this.steps,
    this.physics,
    this.type = FxStepperType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
  ...
  /// [FxStepperType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [FxStepperType.vertical] case where it is
  /// displayed in-between.
  final FxStepperType type;
  ...
  /// The elevation of this stepper's [Material] when [type] is [FxStepperType.horizontal].
  ...
  @override
  State<FxStepper> createState() => _StepperState();
}

class _StepperState extends State<FxStepper> with TickerProviderStateMixin {
  late List<GlobalKey> _keys;
  final Map<int, FxStepState> _oldStates = <int, FxStepState>{};
  ...
  @override
  void didUpdateWidget(FxStepper oldWidget) {
  ...
  bool _isLabel() {
    for (final FxStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index,
              oldState && widget.steps[index].state == FxStepState.error),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == FxStepState.error
            ? CrossFadeState.showSecond
            : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != FxStepState.error) {
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.type) {
      case FxStepperType.vertical:
        return _buildVertical();
      case FxStepperType.horizontal:
